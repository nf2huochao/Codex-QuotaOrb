...
- **Codex 重置预测 / Reset forecast** — 详情页和手机局域网网页显示未来 24 小时重置概率、距上次重置时间、近 30 天重置次数、平均等待时间和最近重置时间，并提供 LunarWerx 公开数据证据链接。 The desktop details page and LAN web view show reset forecast metrics with a public LunarWerx evidence link.
...
- **采样状态区分 / Sampling distinction** — 近期趋势中的真实采样使用橙色细柱，推算或沿用数据使用低饱和绿色，便于区分数据来源。 Real samples use orange bars while inferred or carried-forward values remain muted green.
...
- 继续保持任务状态、Token、套餐、周额度、配对和可用重置机会逻辑不变；预测来源不可访问时只显示明确的不可用状态。
- [下载 v0.1.9 Windows x64 安装包](https://github.com/nf2huochao/Codex-QuotaOrb/releases/download/v0.1.9/Codex._0.1.9_x64-setup.exe) · [查看 GitHub 发行版](https://github.com/nf2huochao/Codex-QuotaOrb/releases/tag/v0.1.9)。构建与测试已验证：`npm run build` 通过，39 项测试全部通过。
- 当前发行包为 Windows x64 测试包；签名自动更新需要在 GitHub Actions 中配置仓库 Secrets，详见 [`docs/release-signing.md`](docs/release-signing.md)。
