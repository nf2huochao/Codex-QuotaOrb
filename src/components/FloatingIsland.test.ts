...
describe('compact floating island', () => {
  ...
  it('uses the five-hour quota for Plus without changing task data', () => { const root = document.createElement('div'); renderFloatingIsland(root, { ...base(), quotaRemainingPercent: 91, fiveHourRemainingPercent: 62 }, vi.fn()); expect(root.textContent).toContain('62%'); expect(root.textContent).toContain('周额度剩余 91%'); expect(root.textContent).toContain('5小时额度可用') })
