...
describe('compact floating island', () => {
  ...
  it('shows a fixed seven-by-twenty-four weekly quota grid without wheel switching', () => { const root = document.createElement('div'); const now = Math.floor(Date.now() / 3600000) * 3600; const history = Array.from({ length: 168 }, (_, index) => ({ at: now - (167 - index) * 3600, quotaRemainingPercent: index % 2 ? 70 : 72 })); renderDetailsPanel(root, { ...base(), history }, vi.fn(), vi.fn(), vi.fn()); const card = root.querySelector<HTMLElement>('.history-card')!; expect(root.querySelector('.history-summary')?.textContent).toContain('周额度'); expect(root.querySelector('.history-window')?.textContent).toBe('最近 7 天'); expect(root.querySelectorAll('.history-grid .history-slot')).toHaveLength(168); expect(root.querySelector('.history-scroll')).toBeNull(); card.dispatchEvent(new WheelEvent('wheel', { deltaY: -100, bubbles: true, cancelable: true })); expect(root.querySelector('.history-window')?.textContent).toBe('最近 7 天'); expect(root.querySelectorAll('.history-grid .history-slot')).toHaveLength(168) })
  it('leaves unsampled weekly quota hours empty instead of carrying old values', () => { const root = document.createElement('div'); const now = Math.floor(Date.now() / 3600000) * 3600; const history = [{ at: now - 3 * 3600, quotaRemainingPercent: 83 }, { at: now - 1 * 3600, quotaRemainingPercent: 79 }]; renderDetailsPanel(root, { ...base(), history }, vi.fn(), vi.fn(), vi.fn()); expect(root.querySelectorAll('.history-grid .history-slot')).toHaveLength(168); expect(root.querySelectorAll('.history-slot.carried')).toHaveLength(0); expect(root.querySelectorAll('.history-slot.has-value')).toHaveLength(2) })
