...
const base = (): Snapshot => ({ status: 'fresh', fetchedAt: 100, quotaRemainingPercent: 72, fiveHourRemainingPercent: 72, quotaResetsAt: 200, fiveHourResetsAt: 300, plan: 'Plus', resetCredits: 1, todayTokens: 128400, activeTaskCount: 3, taskCounts: { none: 0, needsAction: 0, running: 3, completed: 0 }, history: [], previousHistory: [], schemaVersion: '1.0', tasks: [1, 2, 3].map((id) => ({ id: String(id), title: `任务 ${id}`, status: 'running', updatedAt: 100, acknowledged: false })) })

describe('compact floating island', () => {
  it('switches fixed labels to English while preserving task text', () => {
    setLanguage('en-US')
    const root = document.createElement('div')
    renderDetailsPanel(root, { ...base(), tasks: [{ id: 'task', title: '中文对话标题', activity: '正在处理', status: 'running' as const, updatedAt: 100, acknowledged: false }] }, vi.fn(), vi.fn(), vi.fn())
    expect(root.querySelector('.pairing-settings-button')?.textContent).toBe('Pair')
    expect(root.querySelector('.language-toggle')?.textContent).toBe('中')
    expect(root.querySelector('.task-header-label')?.textContent).toBe('Task status')
    expect(root.textContent).toContain('中文对话标题')
    expect(root.textContent).toContain('Current activity')
    setLanguage('zh-CN')
  })
  ...
  it('shows quota ring, dated token label, canonical task count and compact tokens', () => { const root = document.createElement('div'); renderFloatingIsland(root, base(), vi.fn()); expect(root.textContent).toContain('72%'); expect(root.textContent).toContain('3'); expect(root.querySelector('[data-status="running"]')).toBeTruthy(); expect(root.textContent).toContain('Token ·'); expect(root.textContent).toContain('12.8万'); expect(root.querySelector('.quota-ring')).toBeTruthy(); expect(root.querySelector('canvas,img')).toBeNull(); expect(root.querySelector<HTMLElement>('.quota-ring')?.style.getPropertyValue('--quota')).toBe('259.2deg') })
  it('uses the five-hour quota for Plus without changing task data', () => { const root = document.createElement('div'); renderFloatingIsland(root, { ...base(), quotaRemainingPercent: 91, fiveHourRemainingPercent: 62 }, vi.fn()); expect(root.textContent).toContain('62%'); expect(root.textContent).toContain('周剩余 91%'); expect(root.textContent).toContain('5小时额度'); expect(root.textContent).not.toContain('周额度剩余'); expect(root.textContent).not.toContain('5小时额度可用') })
  ...
  it('shows Plus five-hour labels and weekly auxiliary note', () => { const root = document.createElement('div'); renderDetailsPanel(root, { ...base(), quotaRemainingPercent: 91, fiveHourRemainingPercent: 62, quotaResetsAt: 1724972700, fiveHourResetsAt: 1725000000 }, vi.fn(), vi.fn(), vi.fn()); expect(root.querySelector('.details-title')?.textContent).toContain('5小时额度剩余 62%'); expect(root.querySelector('.reset-label')?.textContent).toBe('5小时额度重置时间'); expect(root.querySelector('.weekly-quota-note')?.textContent).toContain('本周剩余 91%'); expect(root.querySelector('.weekly-quota-note')?.textContent).toContain('重置时间为') })
  it('renders a seven-by-twenty-four weekly smooth chart without wheel switching', () => { const root = document.createElement('div'); const now = Math.floor(Date.now() / 3600000) * 3600; const history = Array.from({ length: 168 }, (_, index) => ({ at: now - (167 - index) * 3600, quotaRemainingPercent: index % 2 ? 70 : 72 })); renderDetailsPanel(root, { ...base(), history }, vi.fn(), vi.fn(), vi.fn()); const card = root.querySelector<HTMLElement>('.history-card')!; expect(root.querySelector('.history-summary')?.textContent).toContain('周额度'); expect(root.querySelector('.history-window')?.textContent).toBe('本周期'); expect(root.querySelector('.history-chart')).toBeTruthy(); expect(root.querySelectorAll('.history-hover-slot')).toHaveLength(168); expect(root.querySelectorAll('.history-day-divider')).toHaveLength(8); expect(root.querySelector('.history-line')).toBeTruthy(); expect(root.querySelector('.history-line')?.getAttribute('d')).toContain(' C '); expect(root.querySelectorAll('.history-sample-point,.history-display-point')).toHaveLength(0); expect(root.querySelector('.history-scroll')).toBeNull(); card.dispatchEvent(new WheelEvent('wheel', { deltaY: -100, bubbles: true, cancelable: true })); expect(root.querySelector('.history-window')?.textContent).toBe('本周期'); expect(root.querySelectorAll('.history-hover-slot')).toHaveLength(168) })
  it('carries the last real weekly quota sample across unsampled hours', () => { const root = document.createElement('div'); const now = Math.floor(Date.now() / 3600000) * 3600; const history = [{ at: now - 3 * 3600, quotaRemainingPercent: 83 }, { at: now - 1 * 3600, quotaRemainingPercent: 79 }]; renderDetailsPanel(root, { ...base(), history }, vi.fn(), vi.fn(), vi.fn()); expect(root.querySelectorAll('.history-chart .history-hover-slot').length).toBe(168); expect(root.querySelector('.history-line')).toBeTruthy(); expect(root.querySelectorAll('.history-sample-point,.history-display-point,.history-current-point')).toHaveLength(0) })
  it('switches between the current and previous weekly cycles', () => { const root = document.createElement('div'); const now = Math.floor(Date.now() / 3600000) * 3600; const current = [{ at: now - 3600, quotaRemainingPercent: 79 }]; const previous = [{ at: now - 8 * 24 * 3600, quotaRemainingPercent: 92 }]; renderDetailsPanel(root, { ...base(), history: current, historyCycleKey: now + 3600, previousHistory: previous, previousHistoryCycleKey: now - 7 * 24 * 3600 }, vi.fn(), vi.fn(), vi.fn()); const toggle = root.querySelector<HTMLButtonElement>('.history-toggle')!; expect(toggle.hidden).toBe(false); expect(toggle.textContent).toBe('查看上一周期'); toggle.click(); expect(root.querySelector('.history-window')?.textContent).toBe('上一周期'); expect(toggle.textContent).toBe('返回本周期'); expect(root.querySelector('.history-content')?.textContent).toContain('92%'); toggle.click(); expect(root.querySelector('.history-window')?.textContent).toBe('本周期') })
  it('keeps the previous-cycle button visible when no archived data exists', () => { const root = document.createElement('div'); renderDetailsPanel(root, base(), vi.fn(), vi.fn(), vi.fn()); const toggle = root.querySelector<HTMLButtonElement>('.history-toggle')!; expect(toggle.hidden).toBe(false); toggle.click(); expect(root.querySelector('.history-window')?.textContent).toBe('上一周期'); expect(root.querySelector('.history-content')?.textContent).toContain('无数据') })
  it('keeps the previous-cycle view after a live snapshot update', () => { const root = document.createElement('div'); const mounted = mountDetailsPanel(root, vi.fn(), vi.fn(), vi.fn()); mounted.update(base()); root.querySelector<HTMLButtonElement>('.history-toggle')!.click(); mounted.update({ ...base(), todayTokens: 128401 }); expect(root.querySelector('.history-window')?.textContent).toBe('上一周期'); expect(root.querySelector('.history-content')?.textContent).toContain('无数据') })
