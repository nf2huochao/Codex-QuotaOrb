...
const base = (): Snapshot => ({ status: 'fresh', fetchedAt: 100, quotaRemainingPercent: 72, fiveHourRemainingPercent: 72, quotaResetsAt: 200, fiveHourResetsAt: 300, plan: 'Plus', resetCredits: 1, todayTokens: 128400, activeTaskCount: 3, taskCounts: { none: 0, needsAction: 0, running: 3, completed: 0 }, history: [], schemaVersion: '1.0', tasks: [1, 2, 3].map((id) => ({ id: String(id), title: `任务 ${id}`, status: 'running', updatedAt: 100, acknowledged: false })) })
...
describe('compact floating island', () => {
  ...
  it('shows today first and expands the retained weekly quota history with the wheel', () => { const root = document.createElement('div'); const now = Math.floor(Date.now() / 3600000) * 3600; const history = Array.from({ length: 168 }, (_, index) => ({ at: now - (167 - index) * 3600, quotaRemainingPercent: index % 2 ? 70 : 72 })); renderDetailsPanel(root, { ...base(), history }, vi.fn(), vi.fn(), vi.fn()); const card = root.querySelector<HTMLElement>('.history-card')!; expect(root.querySelector('.history-summary')?.textContent).toContain('周额度'); expect(root.querySelector('.history-window')?.textContent).toBe('当天采样'); expect(root.querySelectorAll('.history-scroll .history-slot').length).toBeGreaterThan(0); expect(root.querySelectorAll('.history-scroll .history-slot').length).toBeLessThanOrEqual(24); card.dispatchEvent(new WheelEvent('wheel', { deltaY: -100, bubbles: true, cancelable: true })); expect(root.querySelector('.history-window')?.textContent).toBe('最近 2 天'); expect(root.querySelectorAll('.history-scroll .history-slot')).toHaveLength(48); for (let index = 0; index < 10; index += 1) card.dispatchEvent(new WheelEvent('wheel', { deltaY: -100, bubbles: true, cancelable: true })); expect(root.querySelector('.history-window')?.textContent).toBe('最近 7 天'); expect(root.querySelectorAll('.history-scroll .history-slot')).toHaveLength(168) })
  it('supports pointer dragging without exposing a native scrollbar', () => { const root = document.createElement('div'); const now = Math.floor(Date.now() / 3600000) * 3600; const history = Array.from({ length: 168 }, (_, index) => ({ at: now - (167 - index) * 3600, quotaRemainingPercent: 70 })); renderDetailsPanel(root, { ...base(), history }, vi.fn(), vi.fn(), vi.fn()); const scroll = root.querySelector<HTMLElement>('.history-scroll')!; scroll.dispatchEvent(new MouseEvent('pointerdown', { clientX: 300, button: 0, bubbles: true })); scroll.dispatchEvent(new MouseEvent('pointermove', { clientX: 200, bubbles: true })); expect(scroll.scrollLeft).toBe(100); scroll.dispatchEvent(new MouseEvent('pointerup', { clientX: 200, bubbles: true })); expect(scroll.classList.contains('is-dragging')).toBe(false) })
  it('carries the last weekly sample across an unsampled hour without persisting it', () => { const root = document.createElement('div'); const now = Math.floor(Date.now() / 3600000) * 3600; const history = [{ at: now - 3 * 3600, quotaRemainingPercent: 83 }, { at: now - 1 * 3600, quotaRemainingPercent: 79 }]; renderDetailsPanel(root, { ...base(), history }, vi.fn(), vi.fn(), vi.fn()); const carried = [...root.querySelectorAll<HTMLElement>('.history-slot.carried')]; expect(carried.length).toBeGreaterThan(0); expect(carried.some((bar) => bar.title.includes('83%') && bar.title.includes('沿用上次采样'))).toBe(true) })
  it('carries the latest prior-day weekly sample into an un采样的当天', () => { const root = document.createElement('div'); const now = Math.floor(Date.now() / 3600000) * 3600; const history = [{ at: now - 25 * 3600, quotaRemainingPercent: 77 }]; renderDetailsPanel(root, { ...base(), history }, vi.fn(), vi.fn(), vi.fn()); expect(root.querySelector('.history-summary')?.textContent).toContain('沿用上次采样'); expect(root.querySelectorAll('.history-slot.carried').length).toBeGreaterThan(0) })
