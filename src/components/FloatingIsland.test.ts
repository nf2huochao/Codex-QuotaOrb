...
describe('compact floating island', () => {
  ...
  it('renders the retained seven-day history in a horizontal track', () => { const root = document.createElement('div'); const history = Array.from({ length: 180 }, (_, index) => ({ at: 1_700_000_000 + index * 3600, quotaRemainingPercent: index % 2 ? 70 : 72 })); renderDetailsPanel(root, { ...base(), history }, vi.fn(), vi.fn(), vi.fn()); expect(root.querySelector('.history-window')?.textContent).toBe('最近 7 天'); expect(root.querySelectorAll('.history-scroll .history-slot')).toHaveLength(168); expect(root.querySelector('.history-scroll .history-track')?.getAttribute('aria-label')).toContain('最近 7 天') })
