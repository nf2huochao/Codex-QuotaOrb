param(
  ...
  [ValidateSet('SessionStart', 'UserPromptSubmit', 'Stop', 'SessionEnd')]
  [string]$EventName
)
...
try {
  ...
  Write-Error 'Codex lifecycle hook payload could not be parsed.' -ErrorAction Continue
}
...
try {
  ...
  $envelope = '{"eventName":' + (ConvertTo-Json $EventName -Compress) + ',"payload":' + $payload + '}'
  Invoke-RestMethod -Method Post `
    -Uri 'http://127.0.0.1:18765/api/hooks/event' `
    -Headers @{ 'X-Codex-Hook-Token' = [string]$pairing.session_token; 'Content-Type' = 'application/json' } `
    -Body $envelope | Out-Null
  $httpStatus = 200
  $delivered = $true
} catch {
